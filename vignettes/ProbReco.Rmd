...
A challenging aspect of optimising the loss function is its stochastic nature.  The technique used to maximise the loss function is *Stochastic Gradient Descent*.  The gradients with respect to $\boldsymbol{G}$ and $\boldsymbol{d}$ are found using algorithmic differentiation which is imlpemented using the Stan Math C++ header only library[see @stan].  The learning rates for each update are determined using the Adam method [see @adam].

## Using `inscoreopt` function

The easiest function to use in the package is `inscoreopt`.  This only requires the data and predicted values.  The probabilistic forecasts are not true forecasts in the sense that they are obtained using in sample information.
...
ten obseravations of data from the full hierarchy can be simulated by
...
Suppose the predicted values are simply randomly generated from a uniform distribution.  The argument `yhat` must be a matrix of the same size as `y`
...
This section describes how to set up the inputs for the `scoreopt` functions in the `ProbReco` package.  These are more easily created by the `purrr` package which can be loaded using:
...
In practice, an actual dataset will need to be converted into this form.  This form of storing data is unconvential but facilitates the use of `purrr` style functions that are used internally in the package.  

### The `prob` argument

The base probabilistic forecast cann general will vary for each period of the training sample.  This information is also stored in a list.  Each list element corresponds to a training observation.  The list elements themselves are functions that simulate from the base probabilistic distribution.
...
The reconciliation parameters enter functions as a single argument.  The first $m$ elements are the elements of $\boldsymbol{d}$.  The remaining elements are the elements of $\boldsymbol{G}$ vectorised (in column-major ordering).  A random value of $\boldsymbol{G}$ can be simulated as 
